...
func handleGetStats(sp *proxy.StreamProxy) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err := json.NewEncoder(w).Encode(buildStats(sp)); err != nil {
...
// buildStats collects a single system statistics snapshot, shared by the stats
// endpoint and the admin websocket broadcaster.
func buildStats(sp *proxy.StreamProxy) StatsResponse {
	totalChannels := 0
	activeStreams := 0
	connectedClients := 0
	activeRestreamers := 0
