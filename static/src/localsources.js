    ...
    container.innerHTML = sources.map((source, index) => `
        <div class="source-item">
            ...
            <div class="mt-4 pt-4 border-t border-kptv-border flex gap-2">
                <button title="Scan" class="px-3 py-1 bg-green-700 hover:bg-green-600 rounded text-sm transition-colors flex items-center space-x-1"
                    onclick="scanLocalSource(${source.id})">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"></path>
                    </svg>
                </button>
                <button title="Edit" class="px-3 py-1 bg-kptv-blue hover:bg-kptv-blue-light rounded text-sm transition-colors flex items-center space-x-1"
